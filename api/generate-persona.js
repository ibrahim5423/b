...
const FALLBACK_PERSONA = {
  ...
  pressure_points: ['Missing Q3 quota targets', 'Board pressure to reduce CAC'],
  difficulty: 'Medium',
...
export default async function handler(req, res) {
  ...
  const sellerCtx = buildSellerContext(additionalDetails)
  ...
  try {
    const completion = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Generate a realistic B2B sales prospect persona based on: ${query.trim()}${sellerCtx}
...
${additionalDetails && sellerCtx ? `CRITICAL — You MUST use the SELLER CONTEXT above. Do NOT ignore it:
...
` : ''}Return ONLY a raw JSON object with no markdown, no backticks, no explanation. Fields:
{
  ...
  "briefing": string | null
    ...
    try {
      ...
      console.error('JSON parse error, using fallback. Raw:', raw.slice(0, 200))
    ...
    if (!Array.isArray(persona.traits) || persona.traits.length !== 3) persona.traits = FALLBACK_PERSONA.traits
    if (!Array.isArray(persona.objections) || persona.objections.length !== 3) persona.objections = FALLBACK_PERSONA.objections
    if (!Array.isArray(persona.pressure_points) || persona.pressure_points.length !== 2) persona.pressure_points = FALLBACK_PERSONA.pressure_points
    if (!['Easy', 'Medium', 'Hard'].includes(persona.difficulty)) persona.difficulty = 'Medium'
